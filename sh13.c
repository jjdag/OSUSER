	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					// RAJOUTER DU CODE ICI
						// on veut envoyer au serveur le buffer contenant l accusation du joueur
						sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
					}
					...
					// RAJOUTER DU CODE ICI
						sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer); // joueur must also send sa selection de joueur pas seulement l objet
					}
					else if ((objetSel!=-1) && (joueurSel!=-1))
					{
					...
					{
						printf("Erreur ! il faut selectionner un joueur ou un objet\n");
					}
				}
				else
				{
		...
		switch (gbuffer[0])
		{
			...
			case 'M':
				...
				printf("Joueur actuel : %d, goEnabled: %d\n", joueurCourant, goEnabled);
				break;
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				int j, obj, val;
				sscanf(gbuffer, "V %d %d %d", &j, &obj, &val);
				tableCartes[j][obj] = val;
				printf("Val tableCartes[%d][%d] = %d\n", j, obj, val); // mise a jour
				break;

			case 'P': {
				int jDisq;
				sscanf(gbuffer, "P %d", &jDisq);
				printf("Le joueur %d a été disqualifié\n", jDisq);
				break;
			}
		}
		synchro=0;
