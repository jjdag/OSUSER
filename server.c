...
void melangerDeck()
{		
		srand(time(NULL)); // randomize seed at each round/deck mixing sinon on tombe sur les memes combis et coupables
        int i;
...
void sendMessageToClient(char *clientip,int clientport,char *mess)
{
    ...
    bcopy((char *)server->h_addr_list[0], // modif here ! used to be just h_addr but deprecated !!
...
int main(int argc, char *argv[])
{
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
					...
					sprintf(reply, "D 0 1 2");
					sendMessageToClient(tcpClients[0].ipAddress, tcpClients[0].port, reply);
					
					// sprintf(reply, "V %d %d %d %d %d %d %d %d", 
					// tableCartes[0][0], tableCartes[0][1], tableCartes[0][2], 
					// tableCartes[0][3], tableCartes[0][4], tableCartes[0][5], 
					// tableCartes[0][6], tableCartes[0][7]); // register string into buffer pointed at par le char * reply (str composition/preparation to send it off to le joueur 1)
					...
					fsmServer=1; // changing l etat de la machine pour commencer la partie mtn que tous les pers sont etablis
				}
				break;
                }
	}
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
       	case 'G': {
			// RAJOUTER DU CODE ICI
			int pl, suspect;
			sscanf(buffer, "G %d %d", &pl, &suspect); // suspect = guiltSel dans code client
			if (suspect == deck[12])
			{
				sprintf(reply, "%d a gagné la partie en trouvant le coupable %d", pl, suspect); // joueur gagne -> accustion correcte
				broadcastMessage(reply);
				resetGame(); // reset le jeu, partie finie
				// return;
			}
			else
			{
				tcpClients[pl].port = -1;
				sprintf(reply, "P %d", pl); // joueur disqualifie
				broadcastMessage(reply);

				int joueursActuels = 0; // check que pas ts les autres joueurs sont deja disq sinon boucle infinie
				int dernierNonDisq = -1;
				for (int i = 0; i < 4; i++) {
				...
				if (joueursActuels == 1) {
				// vainqueur par defaut si ts les autres sont disq
					sprintf(reply, "%d a gagné la partie par défaut (tous les autres joueurs sont disqualifiés)", dernierNonDisq);
					broadcastMessage(reply);
					resetGame(); // reset partie finie
					// return;
				}

				joueurCourant = (joueurCourant + 1) % 4;
				while (tcpClients[joueurCourant].port == -1) {
					joueurCourant = (joueurCourant + 1) % 4;
				}
				sprintf(reply, "M %d", joueurCourant);
				broadcastMessage(reply);
        ...
        case 'O': {
			// RAJOUTER DU CODE ICI
			int pl, obj;
			sscanf(buffer, "O %d %d", &pl, &obj);

			for (int j = 0; j < 4; j++)
			{
				if (j == pl) continue;

				int cnt = tableCartes[j][obj] > 0 ? 1 : 0;

				sprintf(reply, "V %d %d %d", j, obj, cnt);
				sendMessageToClient(tcpClients[pl].ipAddress, tcpClients[pl].port, reply);
			}
			break;
		}
		...
		case 'S': {
			// RAJOUTER DU CODE ICI
			int pl, cible, obj;
			sscanf(buffer, "S %d %d %d", &pl, &cible, &obj); // id, joueurSel, objSel respectivement

			int count = tableCartes[cible][obj];

			sprintf(reply, "V %d %d %d", cible, obj, count);  // mise a jour des tablesCartes de la cible
			sendMessageToClient(tcpClients[pl].ipAddress, tcpClients[pl].port, reply);
			break;
		}
	...
	if (fsmServer == 1) {
		...
		sprintf(reply, "M %d", joueurCourant); // joueur suivant
		broadcastMessage(reply);
	}
    }
    close(newsockfd);
